...
const configValidation = () => {
  ...
  if (config['KEYCLOAK_CLIENT_ID'] && config['KEYCLOAK_CLIENT_SECRET'] && config['KEYCLOAK_REALM'] && config['KEYCLOAK_SERVER_URL']) {
    return config;
  }
  throw new Error('KEYCLOAK_CLIENT_ID, KEYCLOAK_CLIENT_SECRET, KEYCLOAK_REALM, and KEYCLOAK_SERVER_URL are required');
};

const OAUTH_ENDPOINT = (serverUrl, realm) => `${serverUrl}/realms/${realm}/protocol/openid-connect/auth`;
const OAUTH_TOKEN_ENDPOINT = (serverUrl, realm) => `${serverUrl}/realms/${realm}/protocol/openid-connect/token`;
const OAUTH_USER_INFO_ENDPOINT = (serverUrl, realm) => `${serverUrl}/realms/${realm}/protocol/openid-connect/userinfo`;
...
async function keycloakSignIn(ctx) {
  ...
  const endpoint = OAUTH_ENDPOINT(config['KEYCLOAK_SERVER_URL'], config['KEYCLOAK_REALM']);
...
async function keycloakSignInCallback(ctx) {
  ...
  try {
    const tokenEndpoint = OAUTH_TOKEN_ENDPOINT(config['KEYCLOAK_SERVER_URL'], config['KEYCLOAK_REALM']);
    const response = await axios.post(tokenEndpoint, params, {
      headers: {
        'Content-Type': 'application/x-www-form-urlencoded',
      },
    });
    const userResponse = await axios.get(OAUTH_USER_INFO_ENDPOINT(config['KEYCLOAK_SERVER_URL'], config['KEYCLOAK_REALM']), {
      headers: {
    ...
    if (dbUser) {
      ...
      const roles = keycloakRoles && keycloakRoles['roles'] ? keycloakRoles['roles'].map(role => ({ id: role })) : [];
